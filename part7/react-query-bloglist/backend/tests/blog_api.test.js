...
describe("When there is initially some blogs saved", () => {
  ...
  test("blogs are returned as json", async () => {
    // why use async here? Because we are using "await" inside the function, which is an asynchronous operation
  ...
  describe("viewing a specific blog", () => {
    ...
    test("fails with statuscode 404 if blog does not exist", async () => {
      ...
      await api.get(`/api/blogs/${validNonExistingId}`).expect(404); // status code 404 means that the requested resource was not found
    });

    test("fails with statuscode 400 id is invalid", async () => {
      const invalidId = "5a3d5da59070081a82a3445";

      await api.get(`/api/blogs/${invalidId}`).expect(400); // status code 400 means that the request was invalid
